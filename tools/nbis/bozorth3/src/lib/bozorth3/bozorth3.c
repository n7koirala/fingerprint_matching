...
FILE *fp_minutia_pair = fopen("32_matches.txt","w");
FILE *fp_sim_pair = fopen("similarity_pairs.txt","w");

/* Foreach sorted edge in Subject's Web ... */

for ( k = 1; k < probe_ptrlist_len; k++ ) {
	ss = scolpt[k-1];
	fprintf(fp_minutia_pair, "%d; %d <-> %d; (%d, %d) (%d, %d)\n",k, ss[3], ss[4], global_xcol[ss[3]], global_ycol[ss[3]], global_xcol[ss[4]], global_ycol[ss[4]]); 
	...
	for ( j = st; j <= gallery_ptrlist_len; j++ ) {
		...
		if ( n != b ) {
			...
			*rotptr++ = *(ff+3);
			...
			fprintf(fp_sim_pair, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n", p1, global_xcol[ss[3]], global_ycol[ss[3]], global_xcol[ss[4]], global_ycol[ss[4]], server_xcol[ff[4]], server_ycol[ff[4]], server_xcol[ff[3]], server_ycol[ff[3]]);
		} else {
			*rotptr++ = p1;
			*rotptr++ = *(ss+3);
			*rotptr++ = *(ss+4);
			...
			*rotptr++ = *(ff+4);
			...
			fprintf(fp_sim_pair, "%d,%d,%d,%d,%d,%d,%d,%d,%d\n", p1, global_xcol[ss[3]], global_ycol[ss[3]], global_xcol[ss[4]], global_ycol[ss[4]], server_xcol[ff[3]], server_ycol[ff[3]], server_xcol[ff[4]], server_ycol[ff[4]]);
		}
