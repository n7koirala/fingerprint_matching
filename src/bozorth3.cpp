...
int main(int argc, char **argv) {
  ...
  int bflag = 0;
  int lflag = 0;
  int gallery_from_argv =
      1; /* true if gallery files are to be pulled from the command line */
  int probes_from_argv =
      1; /* true if probe   files are to be pulled from the command line */
  int neither_from_argv;
  int both_from_argv;
  int both_fixed; /* true if probe file has been fixed with "-p" and gallery
                     file with "-g" */
  int argcount;
...
#ifdef TESTING
  getopt_spec =
      malloc_or_exit((int)strlen(default_getopt_spec) + 2, "getopt() string");
  strcpy(getopt_spec, default_getopt_spec);
  strcat(getopt_spec, "i");
#else
  getopt_spec =
      malloc_or_exit((int)strlen(default_getopt_spec) + 1, "getopt() string");
  strcpy(getopt_spec, default_getopt_spec);
#endif
  ...
  while (1) {
    ...
    case 'A': {
      ...
      if (strncmp(optarg, A_rfd, strlen(A_rfd)) == 0) {
        ...
        if (stop_read_fd < 0) {
          fprintf(
              stderr,
              "%s: WARNING: negative read file descriptor (%d) is illegal\n",
      ...
      if (strncmp(optarg, A_wfd, strlen(A_wfd)) == 0) {
        ...
        if (stop_write_fd < 0) {
          fprintf(
              stderr,
              "%s: WARNING: negative write file descriptor (%d) is illegal\n",
      ...
      if (strncmp(optarg, A_mm, strlen(A_mm)) == 0) {
        ...
        if (min_computable_minutiae < 0) {
          fprintf(
              stderr,
              "%s: WARNING: negative min_computable_minutiae (%d) is illegal\n",
      ...
      if (strncmp(optarg, A_mf, strlen(A_mf)) == 0) {
        ...
        if (max_list_length < 0) {
          fprintf(stderr,
                  "%s: WARNING: negative max_list_length (%d) is illegal\n",
      ...
      if (strncmp(optarg, A_pl, strlen(A_pl)) == 0) {
        ...
        if (s < 0) {
          fprintf(stderr,
                  "%s: ERROR: bad probe line range specifier \"%s\" (error "
                  "code %d)\n",
      ...
      if (strncmp(optarg, A_gl, strlen(A_gl)) == 0) {
        ...
        if (s < 0) {
          fprintf(stderr,
                  "%s: ERROR: bad gallery line range specifier \"%s\" (error "
                  "code %d)\n",
    ...
    case 'm': /* "-m1" */
      if (strcmp(optarg, "1") != 0) {
        fprintf(stderr,
                "%s: ERROR: illegal -m option (-m%s), \"-m1\" expected\n",
                PROGRAM, optarg);
        ++parse_errors;
      }
      m1_xyt = 1;
    ...
    case 'n':
      if (verbose_main)
        fprintf(stderr,
                "Max minutiae to use from each file specified as \"%s\"\n",
                optarg);
      ...
      if (max_minutiae < MIN_BOZORTH_MINUTIAE) {
        fprintf(
            stderr,
            "%s: WARNING: max_minutiae (%d) < lower limit of %d is illegal\n",
      ...
      if (max_minutiae > MAX_BOZORTH_MINUTIAE) {
        fprintf(
            stderr,
            "%s: WARNING: max_minutiae (%d) > upper limit of %d is illegal\n",
  ...
  if (mates_list != CNULL && probe_list != CNULL) {
    fprintf(stderr,
            "%s: ERROR: flags \"-M\" and \"-P\" are not currently compatible\n",
            PROGRAM);
    ++parse_errors;
  }
  if (mates_list != CNULL && gallery_list != CNULL) {
    fprintf(stderr,
            "%s: ERROR: flags \"-M\" and \"-G\" are not currently compatible\n",
            PROGRAM);
    ++parse_errors;
  }
  ...
  if (gallery_list != CNULL && probe_list != CNULL) {
    fprintf(stderr,
            "%s: ERROR: flags \"-G\" and \"-P\" are not currently compatible\n",
            PROGRAM);
    ++parse_errors;
  }
  ...
  if (no_output && outfile != CNULL) {
    fprintf(stderr,
            "%s: ERROR: flags \"-o\" and \"-A nooutput\" are incompatible\n",
            PROGRAM);
    ++parse_errors;
  }
  ...
  if (threshold_stop_flag && !threshold_set) {
    fprintf(stderr,
            "%s: ERROR: flag \"-q\" and requires that a threshold be set\n",
            PROGRAM);
    ++parse_errors;
  }
  ...
  if (stop_read_fd >= 0 && !threshold_stop_flag) {
    fprintf(stderr,
            "%s: ERROR: if stop read fd is set, the threshold stop flag be "
            "also set\n",
            PROGRAM);
    ++parse_errors;
  }
  if (stop_write_fd >= 0 && !threshold_stop_flag) {
    fprintf(stderr,
            "%s: ERROR: if stop write fd is set, the threshold stop flag be "
            "also set\n",
            PROGRAM);
    ++parse_errors;
  }
  if (stop_read_fd >= 0 ||
      stop_write_fd >=
          0) { /* -p probefile.xyt { gallery*.xyt | -G gallery.lis } */
    if (fixed_probe_file == CNULL) {
      fprintf(stderr,
              "%s: ERROR: options \"-A [rw]fd=#\" requires \"-p\" flag\n",
              PROGRAM);
      ++parse_errors;
    }
    if (fixed_gallery_file != CNULL) {
      fprintf(stderr,
              "%s: ERROR: options \"-A [rw]fd=#\" is incompatible with \"-g\" "
              "flag\n",
              PROGRAM);
      ++parse_errors;
    }
    if (stop_read_fd < 0 || stop_write_fd < 0) {
      fprintf(stderr,
              "%s: ERROR: both options \"-A rfd=#\" and \"-A wfd=#\" must be "
              "set if either is\n",
              PROGRAM);
      ++parse_errors;
    }
  ...
  if (gline_begin > 0 && gallery_list == CNULL) {
    fprintf(stderr,
            "%s: ERROR: option \"-A glines=<range>\" requires \"-G\" flag\n",
            PROGRAM);
    ++parse_errors;
  }
  if (pline_begin > 0 && probe_list == CNULL) {
    fprintf(stderr,
            "%s: ERROR: option \"-A plines=<range>\" requires \"-P\" flag\n",
            PROGRAM);
    ++parse_errors;
  }
  if (lflag && bflag) {
  ...
  if (stop_fds) {
    if (signal(SIGPIPE, SIG_IGN) == SIG_ERR) {
      /* fifo(4): When a process tries to write to a FIFO that is not opened for
  ...
  if (errorfile == CNULL) {
    errorfp = stderr; /* If no error file has been specified, error and verbose
  ...
  if (outdir != CNULL) {
    if (outfile != CNULL) {
      ...
      if (outfile[0] == '/') {
        fprintf(errorfp,
                "%s: ERROR: the output filename \"%s\" cannot be an absolute "
                "pathname when the output directory is specified\n",
      ...
      if (fixed_probe_file != CNULL && fixed_gallery_file == CNULL) {
        ...
        fprintf(errorfp,
                "%s: ERROR: output filename can't be assumed unless either "
                "\"-p\" or \"-g\" is specified (but not both)\n",
                get_progname());
        exit(1);
      }
    }
  ...
  if (mates_list != CNULL) {
    ...
    if (mates_fp == FPNULL) {
      fprintf(errorfp,
              "%s: ERROR: fopen() of mates list file \"%s\" failed: %s\n",
  ...
  if (gallery_list != CNULL) {
    ...
    if (gallery_fp == FPNULL) {
      fprintf(errorfp,
              "%s: ERROR: fopen() of gallery list file \"%s\" failed: %s\n",
  ...
  if (probe_list != CNULL) {
    ...
    if (probe_fp == FPNULL) {
      fprintf(errorfp,
              "%s: ERROR: fopen() of probe list file \"%s\" failed: %s\n",
  ...
  if (neither_from_argv) {
    if (optind < argc) {
      fprintf(errorfp,
              "%s: ERROR: no extra xyt-files can be specified on the command "
              "line when both probes and gallery are set via \"-[PpGg]\"\n",
              get_progname());
      usage(errorfp);
      exit(1);
    }
    ...
    if (argcount <= 0) {
      fprintf(errorfp,
              "%s: ERROR: no xyt-files are specified on the command line\n",
              get_progname());
      usage(errorfp);
      exit(1);
    }
    if (both_from_argv) { /* The command line will therefore contain
                             (probes,gallery) xyt-filename pairs */
      if (argcount % 2 != 0) {
        fprintf(errorfp,
                "%s: ERROR: %d xyt-files specified on the command line; the "
  ...
  if (!no_output && print_scores_at_end) {
    ...
    lines = (char **)malloc_or_exit((int)(max_line_count * sizeof(int *)),
                                    "output score line table");
  }
  ...
  while (1) {
    ...
    if (stop_fds) {
      if (fd_readable(stop_read_fd)) {
        ...
        stop_read_fd = -1;
        fprintf(errorfp,
                "%s: exiting after %d computations; a match was found by a "
                "sibling matcher process\n",
    ...
    p = get_next_file(fixed_probe_file, probe_fp, mates_fp, &done_probe_now,
                      &done_probe_afterwards, &pline[0], argc, argv, &optind,
                      &plineno, pline_begin, pline_end);
    g = get_next_file(fixed_gallery_file, gallery_fp, mates_fp,
                      &done_gallery_now, &done_gallery_afterwards, &gline[0],
                      argc, argv, &optind, &glineno, gline_begin, gline_end);
    ...
    if (done_now) {
      if (verbose_main > 1)
        fprintf(errorfp,
                "breaking main loop now [line_count = %d; %d; %d; %d; %d]\n",
                line_count, done_probe_now, done_gallery_now,
                done_probe_afterwards, done_gallery_afterwards);
      break;
    }
    ...
    ok = 1;
    if (fixed_probe_file ==
        CNULL) { /* unless probe file is fixed, load next probe file */
      pstruct = bz_load(p);
      if (pstruct == XYT_NULL) {
        ++nerrors;
        ok = 0;
    ...
    if (fixed_gallery_file ==
        CNULL) { /* unless gallery file is fixed, load next gallery file */
      gstruct = bz_load(g);
      if (gstruct == XYT_NULL) {
        ++nerrors;
        ok = 0;
    ...
    if (done_after_open_failure) {
      if (verbose_main)
        fprintf(errorfp,
                "breaking main loop after encountering an open failure\n");
      break;
    }
    ...
    if (fixed_probe_file ==
        CNULL) /* unless probe file is fixed, free the last probe file's data */
      if (pstruct != XYT_NULL)
        free((char *)pstruct);

    if (fixed_gallery_file == CNULL) /* unless gallery file is fixed, free the
                                        last gallery file's data */
      if (gstruct != XYT_NULL)
        free((char *)gstruct);
      ...
      if (!no_output) {
        if (print_scores_at_end) {
          ...
          if (line_count >= max_line_count) {
            fprintf(errorfp,
                    "%s: ERROR: maximum scores line count (%d) exceeded\n",
                    get_progname(), max_line_count);
            fprintf(
                errorfp,
                "    (Either increase with \"-A maxfiles=#\", or use \"-l\" or "
                "\"-b\" for line- or block-buffering, respectively)\n");
            exit(1);
          }
          scoreln = get_score_line(p, g, n, 0, outfmt);
          if (scoreln == CNULL) {
            fprintf(errorfp,
                    "%s: ERROR: strdup() for output line %d failed: %s\n",
          ...
          if (fputs(get_score_line(p, g, n, 1, outfmt), outfp) == EOF) {
            fprintf(errorfp,
                    "%s: ERROR: fputs() of the match score line failed\n",
                    get_progname());
            exit(1);
          }
          if (ferror(outfp)) {
    ...
    if (threshold_met && threshold_stop_flag) {
      ...
      if (stop_fds) {
        ...
        fprintf(errorfp,
                "%s: MATCH FOUND after %d computations; match score %d >= "
                "threshold %d\n",
  ...
  if (!no_output) {
    ...
    if (print_scores_at_end) {
      ...
      for (i = 0; i < line_count; i++) {
        ...
        if (fputs(lines[i], outfp) == EOF) {
          fprintf(
              errorfp,
              "%s: ERROR: fputs() of the match score line %d of %d failed\n",
              get_progname(), i + 1, line_count);
          ++nerrors;
          break;
        }
        if (ferror(outfp)) {
          fprintf(errorfp,
                  "%s: ERROR: match score write failure at line %d of %d\n",
                  get_progname(), i + 1, line_count);
          ++nerrors;
          break;
        }
    ...
    /* All match scores have now been printed, regardless of whether they were
     * printed after */
    ...
    if (outfile != CNULL) {
      if (ferror(outfp)) {
        fprintf(errorfp,
                "%s: ERROR: match score write failure on output file \"%s\"\n",
                get_progname(), outfile);
        ++nerrors;
      }
      if (fclose(outfp) != 0) {
        fprintf(errorfp,
                "%s: ERROR: fclose() of output file \"%s\" failed: %s\n",
                get_progname(), outfile, strerror(errno));
        ++nerrors;
      }
    }
  ...
  if (gallery_fp != FPNULL) {
    if (fclose(gallery_fp) != 0) {
      fprintf(errorfp,
              "%s: ERROR: fclose() of gallery list \"%s\" failed: %s\n",
              get_progname(), gallery_list, strerror(errno));
      ++nerrors;
    }
  }
