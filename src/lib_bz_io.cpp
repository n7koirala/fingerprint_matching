...
/***********************************************************************/
void set_probe_filename(char *filename) { pfile = filename; }

/***********************************************************************/
void set_gallery_filename(char *filename) { gfile = filename; }

/***********************************************************************/
char *get_progname(void) { return program_buffer; }

/***********************************************************************/
char *get_probe_filename(void) { return pfile; }

/***********************************************************************/
char *get_gallery_filename(void) { return gfile; }

/***********************************************************************/
char *get_next_file(char *fixed_file, FILE *list_fp, FILE *mates_fp,
                    int *done_now, int *done_afterwards, char *line, int argc,
                    char **argv, int *optind,

                    int *lineno, int begin, int end) {
  char *p;
  FILE *fp;
...
/***********************************************************************/
char *get_score_line(const char *probe_file, const char *gallery_file, int n,
                     int static_flag, const char *fmt) {
  int nchars;
  char *bufptr;
...
struct xyt_struct *bz_load(const char *xyt_file) {
  ...
  int xvals_lng[MAX_FILE_MINUTIAE], /* Temporary lists to store all the minutaie
                                       from a file */
      yvals_lng[MAX_FILE_MINUTIAE], tvals_lng[MAX_FILE_MINUTIAE],
  ...
  while (fgets(xyt_line, sizeof xyt_line, fp) != CNULL) {
    m = sscanf(xyt_line, "%d %d %d %d", &xvals_lng[nminutiae],
               &yvals_lng[nminutiae], &tvals_lng[nminutiae],
               &qvals_lng[nminutiae]);
    ...
    if (nminutiae == 0) {
      ...
      if (m != nargs_expected) {
        fprintf(errorfp,
                "%s: ERROR: inconsistent argument count on line %u of minutiae "
                "file \"%s\"\n",
                get_progname(), nminutiae + 1, xyt_file);
        return XYT_NULL;
      }
  ...
  if (xytq_s == XYTQ_NULL) {
    fprintf(errorfp,
            "%s: ERROR: malloc() failure while loading minutiae buffer failed: "
            "%s\n",
...
struct xyt_struct *bz_prune(struct xytq_struct *xytq_s, int verbose_load) {
  ...
  int xvals_lng[MAX_FILE_MINUTIAE], yvals_lng[MAX_FILE_MINUTIAE],
      tvals_lng[MAX_FILE_MINUTIAE], qvals_lng[MAX_FILE_MINUTIAE];
  int order[MAX_FILE_MINUTIAE];
  int xvals[MAX_BOZORTH_MINUTIAE], yvals[MAX_BOZORTH_MINUTIAE],
...
#define C2 1
  ...
  if (nminutiae > max_minutiae) {
    if (verbose_load)
      fprintf(errorfp,
              "%s: WARNING: bz_prune(): trimming minutiae to the %d of highest "
              "quality\n",
    ...
    for (j = 0; j < nminutiae; j++) {
      if (verbose_load)
        fprintf(errorfp, "   %3d: %3d %3d %3d ---> order = %3d\n", j,
                xvals_lng[j], yvals_lng[j], qvals_lng[j], order[j]);
      ...
      if (j > 0) {
        if (c[j].col[0] < c[j - 1].col[0]) {
          fprintf(errorfp,
                  ...
                  get_progname(), j, c[j].col[0], j - 1, c[j - 1].col[0]);
          return XYT_NULL;
        }
        if (c[j].col[0] == c[j - 1].col[0] && c[j].col[1] < c[j - 1].col[1]) {
          fprintf(errorfp,
                  "%s: ERROR: sort failed: c[%d].col[0]=%d == c[%d].col[0]=%d; "
                  "c[%d].col[0]=%d == c[%d].col[0]=%d\n",
                  get_progname(), j, c[j].col[0], j - 1, c[j - 1].col[0], j,
                  c[j].col[1], j - 1, c[j - 1].col[1]);
          return XYT_NULL;
        }
      }
    }
