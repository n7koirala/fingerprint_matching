...
/* 
	int probe_ptrlist_len,		INPUT:  pruned length of Subject's pointer list 
	int gallery_ptrlist_len		INPUT:  pruned length of On-File Record's pointer list 
	Print out the probe and gallery sizes to see how big they are typically. (Assumed 200)
*/
...
for (k = 1; k < probe_ptrlist_len; k++) {
	...
	for (j = 1; j < gallery_ptrlist_len; j++) {
		...
		ss_dist[ ((k-1)*gallery_ptrlist_len) + j-1 ] = (float)ss[0]; /* subject distance */
		ss_dsqr[ ((k-1)*gallery_ptrlist_len) + j-1 ] = (float)SQUARED(ss[0]); /* subject distance */
		ss_b1[ ((k-1)*gallery_ptrlist_len) + j-1 ]   = (float)ss[1]; /* subject b1 */
		ss_b2[ ((k-1)*gallery_ptrlist_len) + j-1 ]   = (float)ss[2]; /* subject b2 */
		...
		ff_dist[ ((k-1)*gallery_ptrlist_len) + j-1 ] = (float)ff[0]; /* on file distance */
		ff_dsqr[ ((k-1)*gallery_ptrlist_len) + j-1 ] = (float)SQUARED(ff[0]); /* on file distance */
		ff_b1[ ((k-1)*gallery_ptrlist_len) + j-1 ]   = (float)ff[1]; /* on file b1 */
		ff_b2[ ((k-1)*gallery_ptrlist_len) + j-1 ]   = (float)ff[2]; /* on file b2 */
...
/* check conditionals after decryption */
	if (ss_dist[k] == 0) continue;

	if (rr_dist[k] > 0) continue;
	if (rr_b1[k] > 1) continue;
	if (rr_b2[k] > 1) continue;
...
for ( k = 1; k < probe_ptrlist_len; k++ ) {
	...
	for ( j = st; j <= gallery_ptrlist_len; j++ ) {
		...
		if ( ss_n[ ((k-1)*gallery_ptrlist_len) + j-1 ] != ff_b[ ((k-1)*gallery_ptrlist_len) + j-1 ] ) {
		...
		while ( t - b > 1 ) {
			...
			for ( i = 0; i < 3; i++ ) {
				...
				n = SENSE(p1,p2); /* n = -1 if p1 < p2, n = 1 if p1 > p2, n = 0 if p1 == p2 */
